-- =============================================
-- ECHELON TEXAS PORTAL - FULL MIGRATION SCRIPT
-- =============================================
...
CREATE POLICY "Users can update their own message images" ON storage.objects
  FOR UPDATE USING (bucket_id = 'messages' AND auth.uid()::text = (storage.foldername(name))[1]);

CREATE POLICY "Users can delete their own message images" ON storage.objects
  FOR DELETE USING (bucket_id = 'messages' AND auth.uid()::text = (storage.foldername(name))[1]);
